...
public class Lambda02 {
    ...
    public static void maxElBul(List<Integer> sayi){
        Optional<Integer> maxSayi=sayi.
                stream().
                reduce(Math::max); //akisa giren elemanlari action sonrasi tek eleman haline getirir
//terminal islemlerden sonra yeni islem yapilmaz reduce ve forEach terminaldir
...
// Task : List'in cift elemanlarin karelerinin en buyugunu print ediniz
    public static void karelerinEnbuyugu(List<Integer> sayi ){
        System.out.println(sayi.stream().
                filter(Lambda01::ciftBul).
                map(t -> t * t).
                reduce(Math::max));//36

        System.out.println("Daha hizli specific Integer Class" +sayi.stream().
                filter(Lambda01::ciftBul).
                map(t -> t * t).
                reduce(Integer::max));
    }
